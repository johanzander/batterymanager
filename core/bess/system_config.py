...
class BatterySettings(BaseModel):
    ...
    chargeCycleCost: float = Field(
        0.50,
        title="Charge Cycle Cost",
        description="Cost per charge cycle in SEK"
    )
...
class ElectricityPriceSettings(BaseModel):
    ...
    area: AreaCode = Field(
        AreaCode.SE4,
        title="Price Area",
        description="Nordpool price area"
    )
    useActualPrice: bool = Field(
        False,
        title="Use Actual Price",
        description="Whether to use actual electricity price calculations"
    )
...
class SystemConfig(BaseModel):
    """System configuration combining capabilities and default settings."""
    battery_settings: BatterySettings = Field(
        default_factory=BatterySettings,
        title="Battery Settings",
        description="Configurable battery settings"
    )
    electricity_settings: ElectricityPriceSettings = Field(
        default_factory=ElectricityPriceSettings,
        title="Electricity Settings",
        description="Configurable electricity pricing settings"
    )

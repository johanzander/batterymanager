...
class EnergyManager:
    ...
    def _init_energy_storage_dicts(self):
        ...
        self._system_production_total = {}  # Total system production (solar + battery) (kWh)
        self._self_consumption_total = {}  # Self consumption (load - grid import) (kWh)
    ...
    def log_energy_balance(self):
        ...
        if historical_hours:
            ...
            total_energy_out = (
                ...
                - totals["battery_discharge"]  # Subtract discharging from output
            )
    ...
    def _format_and_log_report(
        ...
        for data in hourly_data:
            # Add indicator for predicted values
            indicator = "★" if not data["is_historical"] else " "

            # For partial hours, show star for predicted values
            solar_star = " " if data["is_historical"] else "★"
            grid_star = " " if data["is_historical"] else "★"
            home_star = " " if data["is_historical"] else "★"
            export_star = " " if data["is_historical"] else "★"
            aux_star = " " if data["is_historical"] else "★"
            charge_star = " " if data["is_historical"] else "★"
            discharge_star = " " if data["is_historical"] else "★"
            solar_to_bat_star = " " if data["is_historical"] else "★"
            grid_to_bat_star = " " if data["is_historical"] else "★"

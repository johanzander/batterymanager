...
class BatterySystemManager:
    ...
    def __init__(self, controller=None, price_source=None) -> None:
        ...
        self._initial_soc = None  # Store initial SOC for the day

        # Try to read current schedule from inverter at startup
        if self._controller:
            self._initialize_from_inverter()
    ...
    def _initialize_from_inverter(self):
    ...
    def _fetch_predictions(self):
        ...
        Retrieves prediction data from the energy manager with appropriate
        error handling to ensure the system has valid prediction data.
        """
        try:
    ...
    def _calculate_stored_energy_cost_basis(self, hour, prices, current_soc):
        ...
        - Grid energy: avg price paid + discharge cost portion
        ...
        try:
            ...
            if hour <= 0 or available_energy < 0.5:
                return None
            ...
            try:
                ...
                # Default to 50/50 ratio if data unavailable
                solar_charged_today = grid_charged_today = 1.0
            ...
            discharge_only_cost = (
                self.battery_settings.cycle_cost * 0.5
            )  # Only discharge portion

            # For grid energy, use average price paid plus discharge cost
            grid_energy = available_energy * grid_ratio

            # Get average price for past hours (simplified)
            past_prices = prices[:hour]
            if not past_prices:
                logger.warning("No past prices available, using current price")
                avg_grid_price = prices[0]
            else:
                avg_grid_price = sum(past_prices) / len(past_prices)
...
╚══════════════════════════════════╩══════════════════╝\n"""
    ...
    def log_system_startup(self):
        ...
        try:
            ...
            self._price_manager.log_price_information()

...
BATTERY_CHARGE_CYCLE_COST_SEK = 0.50  # Cost per cycle in SEK
...
def print_to_terminal(df, columns_to_print):
    """Print specified columns of a DataFrame to the terminal."""

    columns = [col for col in columns_to_print if col in df.columns]
    formatted_df = df[columns].round(3).to_string(index=False)
    logger.info("\n%s\n", formatted_df)
